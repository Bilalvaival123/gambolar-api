	...
	try {
		...
		if(req.params) {
			if (!req.params.walletAddress) {
				...
				xmlHttp.open( "GET", 'https://bch-insight.bitpay.com/api/addr/' + walletAddress+'/?noTxList=1', false ); // false for synchronous request
				xmlHttp.send();
	...
	try {
		...
		if(req.params) {
			if (!req.params.hash) {
				...
				xmlHttp.open( "GET", 'https://bch-insight.bitpay.com/api/tx/' + hash, false ); // false for synchronous request
				xmlHttp.send();
				var transactions = JSON.parse(xmlHttp.responseText);
				ResponseData = {
					transaction: {
	...
	try {
		...
		if(req.params) {
			if (!req.params.walletAddress) {
				...
				var xmlHttp = new XMLHttpRequest();
				var walletAddress = req.params.walletAddress;
				xmlHttp.open( "GET", 'https://bch-insight.bitpay.com/api/txs/?address=' + walletAddress, false ); // false for synchronous request
				xmlHttp.send();
				var transactions = JSON.parse(xmlHttp.responseText);
				ResponseData = {
	...
	try {
		if(request.body) {
			...
			if(ValidationCheck == true) {
				...
				var xmlHttp = new XMLHttpRequest();
				xmlHttp.open( "GET", 'https://bch-insight.bitpay.com/api/addr/' + from + '/utxo', false ); // false for synchronous request
				xmlHttp.send();
				var transactions = JSON.parse(xmlHttp.responseText);
				...
				await new Promise((resolve, reject) => {
					axios.post("https://bch-insight.bitpay.com/api/tx/send", {
						"rawtx": tx.serialize()
					}).then(res => {
						console.log(res)
						ResponseData = {
