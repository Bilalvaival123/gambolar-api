		...
		if(request.params) {
			if (!request.params.hash) {
				...
				if (hash.length == 64) {
					...
					xmlHttp.open( "GET", 'https://test-insight.bitpay.com/api/tx/' + hash, false ); // false for synchronous request
					xmlHttp.send();
					...
					var xmlHttp = new XMLHttpRequest();
					xmlHttp.open( "GET", 'https://test-insight.bitpay.com/api/txs/?address=' + hash, false ); // false for synchronous request
					xmlHttp.send();
					var transactions = JSON.parse(xmlHttp.responseText);
					ResponseData = {
			...
			if (!request.params.walletAddress) {
				...
				if (walletAddress.length < 34 || walletAddress.length > 34) {
					...
					if(balanceObj.response == '') {
						...
						console.log(balance);
						var xmlHttp = new XMLHttpRequest();
						xmlHttp.open( "GET", 'https://testnet.blockexplorer.com/api/txs/?address=' + walletAddress, false ); // false for synchronous request
						xmlHttp.send();
						var transactions = JSON.parse(xmlHttp.responseText);
						var date = new Date();
						var timestamp = date.getTime();
	...
	return new Promise(function(resolve, reject) {
		insight.broadcast(txdata.toString(), function (err, returnedTxId) {
			if (err) {
				...
				//data = returnedTxId;
				var xmlHttp = new XMLHttpRequest();
				xmlHttp.open( "GET", 'https://test-insight.bitpay.com/api/tx/' + returnedTxId, false ); // false for synchronous request
				xmlHttp.send();
				var transactions = JSON.parse(xmlHttp.responseText);
				data = {
					transaction: {
						...
						block: transactions.blockheight,
						n_confirmation: transactions.confirmations,
						link: `https://testnet.blockexplorer.com/tx/${transactions.txid}`
					},
					message: "",
					timestamp: transactions.time,
					status: 200,
